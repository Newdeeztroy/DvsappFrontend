  ...
  <div class="row">
    <div class="col-12">
      ...
      <div class="table-responsive">
        <table class="table table-hover table-bordered align-middle compact-table" id="clients-table">
          <thead class="table-header">
            <tr>
              <th @click="sortTable('#')">#</th>
              <th @click="sortTable('id')">ID</th>
              <th @click="sortTable('First_name')">Nombres</th>
              <th @click="sortTable('last_name')">Apellidos</th>
              <th @click="sortTable('address')">Cédula</th>
              <th @click="sortTable('email')">Email</th>
              <th @click="sortTable('charge')">Cargo</th>
              <th>Foto</th>
          ...
          <tbody>
            ...
            <tr v-else v-for="(clie, i) in paginatedData" :key="clie.id">
              <td>{{ currentPageStart + i }}</td>
              <td>{{ clie.id }}</td>
              <td>{{ clie.First_name }}</td>
              <td>{{ clie.last_name }}</td>
              <td>{{ clie.address }}</td>
              <td>{{ clie.email }}</td>
              <td>{{ clie.charge }}</td>
              <td>
              ...
              <td>{{ new Date(clie.created_at).toLocaleDateString() }}</td>
              <td class="text-center">
                <router-link :to="{ path: 'viewClient/' + clie.id }" class="btn btn-info btn-sm mx-1" 
                  :class="{ disabled: clie.deleted_at }" 
                  :disabled="!!clie.deleted_at">
                  <i class="fa-solid fa-eye"></i> 
                </router-link>
                <router-link :to="{ path: 'editClient/' + clie.id }" class="btn btn-warning btn-sm mx-1"
                  :class="{ disabled: clie.deleted_at }" 
                  :disabled="!!clie.deleted_at">
                ...
                <button
                  ...
                  @click="deleteclie(clie.id, clie.First_name)"
                  v-if="!clie.deleted_at"
                >
                  <i class="fa-solid fa-trash"></i>
                </button>
                <button
                  class="btn btn-success btn-sm mx-1"
                  @click="restoreClient(clie.id, clie.First_name)"
                  v-if="clie.deleted_at"
                >
...
export default {
  ...
  methods: {
    ...
    exportToPDF() {
      ...
      const rows = this.clients.map((clie, i) => [
        i + 1,
        clie.id,
        clie.First_name,
        clie.last_name,
        ...
        clie.email,
        clie.charge,
        new Date(clie.created_at).toLocaleDateString(),
      ]);
